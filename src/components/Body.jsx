...
const Body = () => {
  ...
  const headerBeforeCss = `before:content-[''] lg:before:h-[37.95rem] before:w-full before:absolute before:inset-0 before:z-[-1] before:bg-cover before:bg-center 
  ${deviceType != 'mobile' 
    ...
    : `before:bg-[url('https://image.tmdb.org/t/p/w500//aFRDH3P7TX61FVGpaLhKr6QiOC1.jpg')]`} `

  const headerAfterCss  = " after:content-[''] after:w-full after:absolute after:top-0 after:h-[92vh] lg:after:h-[37.9rem] after:bg-black/40 after:z-[-1]"

  return moviesList === null ? <ShimmerUI/> : <>
    <SideBar isSideBar={isSideBarShow} setSideBar={setIsSideBarShow } />

    <header className ={`relative 
      ${deviceType != 'mobile' && `${headerAfterCss}`}
      ${!user ? "lg:after:h-[37.95rem] pb-[5rem]" : gpt ? "lg:after:h-[37.95rem]" : "lg:after:h-[42rem]"}
      ${user ? (gpt ? "" : "lg:h-[42rem] h-[32rem]") : ""}
      ${ 
        deviceType != 'mobile' 
        ? !user || (user && gpt) && `${headerBeforeCss}` 
        : `${headerBeforeCss} before:h-[92vh] ${headerAfterCss}`
      } 
    `}>
